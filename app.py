...
# Replace with your real API key for currency if needed
...
@app.route('/basic_calculate', methods=['POST'])
def basic_calculate():
    data = request.get_json()
    expression = data['expression']
    
    try:
        # Simple arithmetic operations allowed
        allowed = {'+': lambda x, y: x + y, '-': lambda x, y: x - y,
                   '*': lambda x, y: x * y, '/': lambda x, y: x / y}
        
        # Ensure the expression is safe and follows basic operations
        if any(op in expression for op in allowed.keys()):
